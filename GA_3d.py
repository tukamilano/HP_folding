...
def rotate90deg(original, point, times):
    ...
    frag -= array[point]
    if times==4:
        frag = frag @ np.array([[1,0,0],[0,0,1], [0,-1,0]])
    elif times==5:
        for _ in range(3):
            frag = frag @ np.array([[1,0,0],[0,0,1], [0,-1,0]])
    else:
        for _ in range(times):
            frag = frag @ np.array([[0,1,0],[-1,0,0],[0,0,1]])
    frag += array[point]
    array[point+1:] = frag
    return array
...
class HP(Chromosome):
    ...
    def get_fitness(self, ignore=False) -> int:
        ...
        for i in range(len(array)):
            for j in range(i+1, len(array)):
                dist = abs(array[i][0] - array[j][0]) + abs(array[i][1] - array[j][1] + abs(array[i][2] - array[j][2]))
                if (dist == 0):
    ...
    def make_random_instance(cls, hp) -> HP:
        array = [[i,0,0] for i in range(len(hp))]
    ...
    def mutate(self, c: float) -> None:
        ...
        for i in range(len(self.array)):
            ...
            rand_rot = [i+1 for i in range(5)]
            shuffle(rand_rot)
            for _ in range(len(rand_rot)):
    ...
    def exec_crossover(self, other: HP, c: float) -> List[HP]:
        ...
        for x in range(len(rand_pivots)):
            ...
            other_head += child1_joint - other_head[-1] - np.array([1,0,0])
            grand_head += child1_joint - grand_head[-1] - np.array([1,0,0])
            if (random() < 0.1):
                arr1[:pivot] = grand_head
            else:
                arr1[:pivot] = other_head

            rand_rot1 = [i for i in range(6)]
            shuffle(rand_rot1)
            ...
            self_tail += child2_joint - self_tail[0] + np.array([1,0,0])
            grand_tail += child2_joint - grand_tail[0] + np.array([1,0,0])
            if (random() < 0.1):
                arr2[pivot:] = grand_tail
            else:
                arr2[pivot:] = self_tail
            rand_rot2 = [i for i in range(6)]
            shuffle(rand_rot2)
...
# hp = "HPHPPHHPHPPHPHHPPHPH"; id="20a" #optimal: 11
hp = "PPHPPHHPPHHPPPPPHHHHHHHHHHPPPPPPHHPPHHPPHPPHHHHH"; id="48a" #optimal: 29
population = 300
generation = 300
...
logging.basicConfig(level=logging.WARNING, 
                    ...
                    style='{', 
                    filename=f"/home/u01230/midterm/results/3d/{id}/{timestamp}.log",
                    filemode='w')
